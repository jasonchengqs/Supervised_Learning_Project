...
clf_dt = DecisionTreeClassifier(min_samples_split=34)
...
param_grid = {'min_samples_split': [2**x for x in range(1,10)]}
...
for score in scores:
    ...
    clf = GridSearchCV(DecisionTreeClassifier(criterion="entropy", class_weight="balanced"), param_grid, cv=10,
                       scoring='%s' % score)
    clf.fit(X_train, y_train)
...
train_sizes, train_scores, val_scores = learning_curve(\
  estimator=DecisionTreeClassifier(min_samples_split=mss), \
